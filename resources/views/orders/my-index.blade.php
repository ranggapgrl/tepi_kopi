...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
    ...
    <div class="mb-8">
        <h1 class="text-2xl sm:text-3xl font-extrabold text-amber-950 tracking-tight mb-2">Pesanan Saya</h1>
        <p class="text-amber-700/80 text-sm">Riwayat dan status semua pesanan yang pernah kamu buat.</p>
    </div>
    ...
    @if($orders->isEmpty())
        <div class="bg-white rounded-2xl border border-amber-100 shadow-sm p-12 text-center">
            ...
            <p class="text-sm text-gray-500 mb-6">Yuk mulai belanja kopi favoritmu di katalog kami.</p>
            ...
            <a href="{{ route('orders.myShow', $order) }}"
               class="block bg-white rounded-2xl border border-amber-100 shadow-sm p-5 sm:p-6 hover:border-amber-300 transition-colors">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        <p class="font-bold text-amber-950">#ORD-{{ str_pad($order->id, 3, '0', STR_PAD_LEFT) }}</p>
