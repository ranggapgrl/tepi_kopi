    ...
    <div class="flex-grow space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-amber-100 shadow-sm p-6 sm:p-8 max-w-3xl">
            <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data" class="space-y-5"
                  x-data="{ preview: {{ $product->image ? "'" . asset('storage/' . $product->image) . "'" : 'null' }} }">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <label for="image"
                        ...
                        <template x-if="!preview">
                            <div class="flex flex-col items-center gap-2 text-amber-500">
                                ...
                                <span class="text-xs font-semibold text-amber-700">Klik untuk ganti gambar</span>
                    ...
                    <p class="text-[11px] text-gray-400 mt-1.5">Kosongkan jika tidak ingin mengganti gambar.</p>
                </div>
